    ...
    <el-form :model="value" ref="productAttrForm" label-width="120px" class="form-inner-container" size="small">
      <el-form-item label="商品规格：">
        <el-card shadow="never" class="cardBg">
          <el-input v-model="guigeName"></el-input>
          <el-button type="primary" @click="addProductName">添加规格名称</el-button>
        ...
        <el-table style="width: 100%; margin-top: 20px" :data="value.skuStockList" border>
          ...
          <el-table-column label="成本价格" width="100" align="center">
            <template slot-scope="scope">
  ...
  methods: {
    addProductName() {
      ...
      for (let i = 0; i < this.selectProductAttr.length; i++) {
        //编辑状态下获取选中属性
        this.selectProductAttr[i].values = this.getEditAttrValues(i)
      }
    },
    handleEditCreated() {
      //根据商品属性分类id获取属性和参数
    ...
    getEditAttrValues(index) {
      ...
      if (index === 0) {
        for (let i = 0; i < this.value.skuStockList.length; i++) {
          ...
          let variants = JSON.parse(sku.variants)
          if (variants != null && variants.length >= 1) {
            values.add(variants[0].value)
          }
        }
      } else if (index === 1) {
        for (let i = 0; i < this.value.skuStockList.length; i++) {
    ...
    getProductSkuSp(row, index) {
      ...
      if (variants != null && index < variants.length) {
    ...
    handleSyncProductSkuStock() {
      this.$confirm('将同步第一个sku的库存到所有sku,是否继续', '提示', {
        ...
        if (this.value.skuStockList !== null && this.value.skuStockList.length > 0) {
          let tempSkuList = []
          tempSkuList = tempSkuList.concat(tempSkuList, this.value.skuStockList)
          let quantity = this.value.skuStockList[0].quantity
          let lowStock = this.value.skuStockList[0].lowStock
          for (let i = 0; i < tempSkuList.length; i++) {
            tempSkuList[i].quantity = quantity
    ...
    refreshProductSkuList() {
      ...
      //只有一个属性时
      console.log('this.selectProductAttr', this.selectProductAttr)
      if (this.selectProductAttr.length === 1) {
        let attr = this.selectProductAttr[0]
        for (let i = 0; i < attr.values.length; i++) {
          skuList.push({
            variants: JSON.stringify([{ key: attr.name, value: attr.values[i] }]),
          })
        }
        ...
        for (let i = 0; i < attr0.values.length; i++) {
          if (attr1.values.length === 0) {
            skuList.push({
              variants: JSON.stringify([{ key: attr0.name, value: attr0.values[i] }]),
            })
            continue
          }
          for (let j = 0; j < attr1.values.length; j++) {
            let variants = []
            variants.push({ key: attr0.name, value: attr0.values[i] })
            variants.push({ key: attr1.name, value: attr1.values[j] })
            skuList.push({
        ...
        for (let i = 0; i < attr0.values.length; i++) {
          ...
          for (let j = 0; j < attr1.values.length; j++) {
            ...
            for (let k = 0; k < attr2.values.length; k++) {
              ...
              variants.push({ key: attr0.name, value: attr0.values[i] })
              variants.push({ key: attr1.name, value: attr1.values[j] })
              variants.push({ key: attr2.name, value: attr2.values[k] })
              skuList.push({
                variants: JSON.stringify(variants),
              })
            }
    ...
    getProductSkuPic(name) {
      for (let i = 0; i < this.value.skuStockList.length; i++) {
        let variants = JSON.parse(this.value.skuStockList[i].variants)
        if (name === variants[0].value) {
    ...
    mergeProductAttrPics() {
      for (let i = 0; i < this.selectProductAttrPics.length; i++) {
        for (let j = 0; j < this.value.skuStockList.length; j++) {
          let variants = JSON.parse(this.value.skuStockList[j].variants)
          if (variants[0].value === this.selectProductAttrPics[i].name) {
